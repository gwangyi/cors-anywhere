...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    hostAllowlist: [],              // Only requests to given host is allowed. Empty means all hosts are allowed.
  };
  ...
  return function(req, res) {
    ...
    if (corsAnywhere.hostAllowlist.length !== 0 && corsAnywhere.hostAllowlist.indexOf(location.hostname) === -1) {
      res.writeHead(403, 'Forbidden host', cors_headers);
      res.end('Host ' + location.hostname + ' is forbidden');
      return;
    }
